...
async def next_page(bot, query):
    ...
    if settings['button']:
            if URL_MODE is True:
                if query.from_user.id in ADMINS:
                    ...
                    btn = [
                        [
                        InlineKeyboardButton(
                            text=f"[{get_size(file.file_size)}] {file.file_name}", url=await (f"https://telegram.me/{BOT_USERNAME}?start=files_{file.file_id}")
                        ),
                        ]
                        for file in files
                    ]
        ...
        if URL_MODE is True:
            if query.from_user.id in ADMINS:
                ...
                btn = [
                    [
                        InlineKeyboardButton(text=f"{file.file_name}", url=await (f"https://telegram.me/{BOT_USERNAME}?start=files_{file.file_id}")),
                        InlineKeyboardButton(text=f"{get_size(file.file_size)}", url=await (f"https://telegram.me/{BOT_USERNAME}?start=files_{file.file_id}")),
                    ]
                    for file in files
                ]
        else:
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/FF_Help/9')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @Film_Freakers ♥️')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('♥️ Thank You Film_Freakers ♥️')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await client.send_message(
                    ...
                    text=f"**{f_caption}** \n\n • Click below links to watch online or Download👇👇",
                    reply_markup=lazy_keyboard,
                    protect_content=True if ident == "filep" else False 
                )
                await query.answer('Requested file url has been sent to you privately. Check PM sweetheart ❤', show_alert=True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔔 Updates 🤖', url='https://t.me/Film_Freakers')
        ], [
            InlineKeyboardButton('🙆🏻 Help 🦾', callback_data='help'),
            InlineKeyboardButton('♥️ About ♥️', callback_data='about')
        ],[
            InlineKeyboardButton('Watch Tutorial', url='https://t.me/FF_Help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer('♥️ Thank You Film_Freakers ♥️')
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://telegram.me/Film_Freakers")
            ],[
                InlineKeyboardButton(text=f"⚡️ Learn Bot Usage 🦋", url=f"https://telegram.me/FF_Help")
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/FF_Help/9'),
        ] 
    )
    if offset != "":
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("The requested content is not available. Please check your spelling correctly in google, If it is correct, have some patience 🙂 - our admin will upload it as soon as possible \n")
        await asyncio.sleep(10)
        await k.delete()
        return
